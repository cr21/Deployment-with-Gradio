...
class ImageClassifier:
    ...
    def predict_batch(self, image_list, progress=gr.Progress(track_tqdm=True)):
        ...
        if not image_list or image_list[0] is None:
            return [[{"none": 1.0}]]
        progress(0.1, desc="Start PreProcessing images")
        tensors=[]
        ...
        for image in image_list:
            ...
            img_tensor = self.transforms(img).to(self.device)
        ...
        progress(0.6, desc="Porcessing predictions")
        batch_results = []
        for probs in predictions:
            top5_prob, top5_label = torch.topk(probs, k=5)
            result = {
                self.labels[idx.item()]:float(prob)
                for prob, idx in zip(top5_prob, top5_label)
            }
...
def main(cfg: DictConfig):
    ...
    demo = gr.Interface(
        ...
        title=f"Advanced   Food Image Classification with {cfg.gradio_deployment.name}",
        description=f"Upload an food image Batch classify it using the {cfg.gradio_deployment.base_model} model",
        batch=True,
        max_batch_size=8,
        example_labels=['pizza','apple pie'],
        examples=[
